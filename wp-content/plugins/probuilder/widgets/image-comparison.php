...
class ProBuilder_Image_Comparison_Widget extends ProBuilder_Base_Widget {
    ...
    protected function render() {
        $s = $this->get_settings();
        $id = 'compare-' . uniqid();
        echo '<div id="' . $id . '" class="pb-image-comparison" style="position:relative;overflow:hidden;user-select:none"><img src="' . esc_url($s['before_image']) . '" style="width:100%;display:block"><div class="after-img" style="position:absolute;top:0;left:0;height:100%;width:' . $s['position'] . '%;overflow:hidden"><img src="' . esc_url($s['after_image']) . '" style="width:100vw;max-width:none"></div><div class="slider" style="position:absolute;top:0;left:' . $s['position'] . '%;width:4px;height:100%;background:#fff;cursor:ew-resize;box-shadow:0 0 10px rgba(0,0,0,0.5)"></div></div>';
